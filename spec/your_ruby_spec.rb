...
RSpec.describe 'YourRuby' do
  ...
  describe '.calculate_completion_time' do
    ...
    it 'calculates when my prints will be ready if they take 2 hours to process and I order them at 16:30 on Thursday' do
      placed_time                = Time.parse('2014-12-04 16:30 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 2, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2014-12-05 10:00 UTC'))
    end  

    it 'calculates when my prints will be ready if they take 10 hours to process and I order them at 12:30 on Thursday' do
      placed_time                = Time.parse('2014-12-04 12:30 UTC')
      calculated_completion_time = YourRuby.calculate_completion_time(placed_time, 11, opening_hours)
      expect(calculated_completion_time).to eq(Time.parse('2014-12-05 15:00 UTC'))
    end  
  end

  describe '.duckduckwhy' do
    it 'gets the top 5 results for "why-the-lucky-stiff" from duckduckgo.com' do
      results = YourRuby.duckduckwhy('why-the-lucky-stiff', 5)
      expect(results).not_to be_nil
      expect(results.size).to eq(5)
      expect(results).to include('https://en.wikipedia.org/wiki/Why_the_lucky_stiff')
    end

    it 'gets the first result for "why-the-lucky-stiff" from duckduckgo.com' do
      results = YourRuby.duckduckwhy('why-the-lucky-stiff', 1)
      expect(results).not_to be_nil
